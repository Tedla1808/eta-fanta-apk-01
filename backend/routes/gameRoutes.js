// --- backend/routes/gameRoutes.js --- (REWRITTEN WITH GAME MODEL)
...
const Game = require('../models/game'); // <-- Import new Game model
...
async function getOrCreateActiveGame(slotId, session) {
    ...
    // If no active game, find the highest round number to create the next one.
...
router.post('/bet', protect, async (req, res) => {
    ...
    try {
        ...
        let totalCost = 0;
        const betsToCreate = [];
        const gamesInvolved = new Map(); // slotId -> game document
