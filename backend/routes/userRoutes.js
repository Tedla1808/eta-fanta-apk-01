// --- backend/routes/userRoutes.js --- (CLEANED UP)
...
router.post('/request-deposit-verification', protect, async (req, res) => {
    try {
        ...
        const user = await User.findById(req.user.id);
        if (!depositorPhone || !amount || amount <= 0) { return res.status(400).json({ message: "Valid phone and amount are required." }); }
        ...
        if (process.env.ADMIN_TELEGRAM_ID && process.env.TELEGRAM_BOT_TOKEN) {
            await axios.post(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`, {
                ...
                text: adminMessage,
                reply_markup: {
                    inline_keyboard: [[{ text: "✅ Verify Deposit", callback_data: `verify-deposit_${transaction._id}` }, { text: "❌ Reject Deposit", callback_data: `reject-deposit_${transaction._id}` }]]
                }
            });
        }
