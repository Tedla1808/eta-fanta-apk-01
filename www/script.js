// --- START OF FILE script.js --- (COMPLETE AND CLEANED UP)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    DOM.saveProfileBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/api/user/profile`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ fullName: DOM.fullNameInput.value }) });
            const data = await response.json();
            if (!response.ok) throw new Error(data.message);
